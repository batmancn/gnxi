...
func parseTestcaseBgpNas(val *pb.TypedValue) error {
    ...
    for i, na := range(nas.GetNa()) {
        fmt.Println("BGP Entry: ", i, ", GetRemoteAddr=" + na.GetRemoteAddr() +
            ", GetLocalAddr=" + na.GetLocalAddr() + ", GetAdminState=" + na.GetAdminState() +
            ", GetAsn=" + na.GetAsn() + ", GetActivateState=" + na.GetActivateState())
    }
...
func parseTestcaseIntfAddr(val *pb.TypedValue) error {
...
func main() {
    ...
    for _, xPath := range xPathFlags {
        ...
        if checkYangPath(getFullPath(pbPath)) == -1 {
